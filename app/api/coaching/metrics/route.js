...
export async function POST(req) {
  try {
    ...
    const { slug, body_weight, waist, notes } = await req.json();

    const [client] = await sql`SELECT id FROM clients WHERE slug = ${slug} LIMIT 1`;
    if (!client) return Response.json({ error: 'not found' }, { status: 404 });

    const [row] = await sql`
      INSERT INTO metrics (client_id, body_weight, waist, notes)
      VALUES (${client.id}, ${body_weight || null}, ${waist || null}, ${notes || null})
